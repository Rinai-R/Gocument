...
func OnlyRead(c context.Context, ctx *app.RequestContext, DocumentId int, conn *websocket.Conn) {
	...
	case ErrCode.OK:
		...
		Document := &models2.ElasticDocument{
		...
		//保存当前连接
		if models2.Conns[DocumentId] == nil {
			Logger.Logger.Debug("Api: not Connected")
			models2.Conns[DocumentId] = make(map[*websocket.Conn]bool)
		}
		models2.Conns[DocumentId][conn] = true // 将连接添加到相应文档组
		//退出之前连接取消
		defer func() {
			delete(models2.Conns[DocumentId], conn)
...
func ReadAndWrite(c context.Context, _ *app.RequestContext, DocumentId int, conn *websocket.Conn) {
	...
	case ErrCode.OK:
		...
		for {
			...
			//此处将要向服务端发送保存的信息
			var document models2.ElasticDocument
			//将读取的信息绑定在结构体上
...
func broadcast(DocumentId int, mes []byte) {
	clients, exists := models2.Conns[DocumentId]
	if !exists {
		return
	}
